...
function surface_source(particle::Particle,source::Surface_Source,cross_sections::Cross_Sections,geometry::Geometry,solver::Solver)
    ...
    if solver isa SN
        ...
        if is_direct_surface
            ...
            Np,_,_,nplus_to_n,n_to_nplus,pl,pm = direct_surface_angular_basis_1D(Ω)
        else
    ...
    if geometry_type == "cartesian" 
        if Ndims == 1
            ...
            if Qdims == 1
                ...
                # Direct collocation for a beam that is exactly a Lobatto node.
                # Its ordinate value is intensity / w, so its angular integral is
                # the requested intensity; no half-range moment inversion is used.
                if is_direct_surface
                    norm = intensity
                    if surface == "X-" && μs > 0
                        n = argmin(abs.(Ω[1] .- μs))
                        abs(Ω[1][n] - μs) ≤ 100eps(Float64) || error("galerkin-direct requires the source direction to be a quadrature node.")
                        np = n_to_nplus[1][n]
                        np != 0 || error("Source direction is not incoming on X-.")
                        Q[source.energy_group,np,1] = intensity / w[n]
                    elseif surface == "X+" && μs < 0
                        n = argmin(abs.(Ω[1] .- μs))
                        abs(Ω[1][n] - μs) ≤ 100eps(Float64) || error("galerkin-direct requires the source direction to be a quadrature node.")
                        np = n_to_nplus[2][n]
                        np != 0 || error("Source direction is not incoming on X+.")
                        Q[source.energy_group,np,2] = intensity / w[n]
                    end
